...
public class HomeController  extends Controller {
    ...
    public Result changeGroup(String groupId) {
        if (groupId.equals(SessionUser.INDIVIDUAL))
            groupId = "";
        int vC = SessionUser.validateSessionUser(session("userId"), groupId, false);
        if (vC == SessionUser.U_VALID) {
            session("groupId", groupId);
            return ok();
        }
        return badRequest(Util.getJSONObj("Invalid group id for user"));
    }
